...
namespace sensor {
AnalogInterface::AnalogInterface(uint32_t channels)
...
float ADS1X15Interface::measure_volts(uint32_t channel)
{
    ...
    return m_ADC.computeVolts(m_ADC.readADC_SingleEnded(channel));
};
#endif
...
#if ESP32
    /**
     * @note O ESP32 realiza correções de erro nos canais analógicos ao usar a
     * função `analogReadMilliVolts` ao invés de `analogRead`, linearizando o
     * resultado.
     */
